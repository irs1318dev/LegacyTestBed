...
public class AutonomousRunner extends RobotComponentBase {
	...
	public void autonomousPeriodic() {
		if (task == null) {
			...
			if (ReferenceData.getInstance().getDipSwitchData()
					.getDipSwitch(DipSwitchRef.DIP_SWITCH_R)
					&& ReferenceData.getInstance().getDipSwitchData()
							.getDipSwitch(DipSwitchRef.DIP_SWITCH_L)) {
				System.out.println("Switch 2 on && switch 1 on - center");
				this.task = new AutonomousEllensburgMiddle();
			} else if (ReferenceData.getInstance().getDipSwitchData()
					.getDipSwitch(DipSwitchRef.DIP_SWITCH_R)) {
